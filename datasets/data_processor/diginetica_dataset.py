...
# 该函数接收的数据是一个列表，列表中的每个元素是一个包含两个列表的元组：seq（项目ID列表）和 cat（类别列表）
# from datasets.data_processor.data_statistics import data_statistics
...
# 每个序列包含三个部分：seq（项目ID列表）、cate（类别列表）和 times（时间列表）。
...
if __name__ == '__main__':
    data_path = '../data/diginetica_x'
    # with open(data_path + '/test.pkl', 'rb') as f:
    #     test_data = pickle.load(f)
    # print(test_data)
    # {
    #     user_id_1: [
    #         ([item_id_1, item_id_2, ...], [category_1, category_2, ...], [time_1, time_2, ...]),
    #         ...
    #     ],
    #     user_id_2: [
    #         ([item_id_3, item_id_4, ...], [category_3, category_4, ...], [time_3, time_4, ...]),
    #         ...
    #     ],
    #     ...
    # }

    all_data, max_vid, item_cates = load_cd_data(data_path, type='aug', test_length=True, highfreq_only=True)
    # [uid, [sequence of items], [label], [sequence of categories], [label category]]
    # print(all_data['train'][-20:])
    # print(len(item_cates))
    data_statistics(all_data)

    csv_file_path = data_path + '/train_data.csv'
    save_train_data_to_csv(all_data['train'], csv_file_path)

    csv_file_path = data_path + '/test_data.csv'
    save_train_data_to_csv(all_data['test'], csv_file_path)
